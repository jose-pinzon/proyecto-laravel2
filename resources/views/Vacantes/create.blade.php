...
@endsection
@section('content')
    <h1 class="text-2xl text-center mt-10">crea tus vacates</h1>

    <form action="{{route('vacantes.store')}}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto my-10">
        ...
        @csrf
            <div class="mb-5">
                ...
                value="{{ old('titulo') }}"  autocomplete="titulo" autofocus placeholder="titulo de la vacante">
        ...
        <div class="mb-5">
            ...
            <div class="mb-5">
                ...
                <select name="experiencia" id="experiencia" class="block appearance-none w-full border-gray-200 text-gray-700 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-100 w-full">
                    ...
                    @foreach ($Experiencias as $exp)
                        <option {{ old('experiencia') == $exp->id ? 'selected' : '' }}  value="{{$exp->id}}">{{$exp->nombre}}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <select name="salario" id="salario" class="block appearance-none w-full border-gray-200 text-gray-700 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-100 w-full">
                    ...
                    @foreach ($salarios as $s)
                        <option {{ old('salario') == $s->id ? 'selected' : '' }} value="{{$s->id}}">{{$s->nombre}}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                @endphp
                <lista-skills :skills="{{json_encode($skills)}}" :oldskills="{{ json_encode(old('habilidad'))}}"></lista-skills>
